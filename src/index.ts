...
export function normalizeEntityName(raw: string): string {
  if (typeof raw !== "string") {
    throw new TypeError(
      "normalizeEntityName: input must be a non-null string"
    );
  }
  ...
  s = s.replace(
    /\b(revocable|irrevocable|living|family)?\s*trust\b/g,
    (_match, qualifier) => {
  ...
  s = s.replace(
    /\blimited\s+liability\s+(company|co)\b/g,
    "llc"
  );
  ...
  s = s.replace(
    /\b(limited\s+partnership|ltd\s+partnership|limited\s+ptnrship|ltd\s+ptnrship|l\s+p)\b/g,
    "lp"
  );
 ...
 */
const OWNER_GROUP_NAME_SUFFIXES: ReadonlySet<string> = new Set([
  "jr", "sr", "ii", "iii", "iv", "v",
 ...
 */
function stripOwnerNameSuffix(name: string): string {
 ...
 */
function normalizeOwnerComponent(raw: string): string {
  return raw
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/[^a-z0-9]+/g, " ")
 ...
 */
function isBlankOrInitial(token: string): boolean {
 ...
 */
export function normalizeOwnerGroupKey(
  ...
  if (!first || isBlankOrInitial(first)) return null;
  if (!lastRaw) return null;

  const lastClean = stripOwnerNameSuffix(lastRaw);
  if (!lastClean || isBlankOrInitial(lastClean)) return null;
  ...
  if (!firstNorm || !lastNorm || !mailingNorm) return null;
...
const CLASSIFIER_RULES: ReadonlyArray<ClassifierRule> = [
  ...
  {
    pattern:
      /\b(limited\s+partnership|ltd\s+partnership|limited\s+ptnrship|ltd\s+ptnrship|family\s+limited\s+partnership|llp|l\.l\.p\.?|lllp|partnership|l\s+p|lp)\b/,
    type: "PARTNERSHIP",
  },
  {
    pattern: /\btrust\b/,
  ...
  },
  {
    ...
    pattern:
      /\b(incorporated|incorp|inc|corporation|corp|company|co|ltd|limited|pc|p\.c\.?)\b/,
    type: "CORPORATION",
  },
  {
 ...
 */
export function classifyEntityType(rawName: string): EntityType {
  if (typeof rawName !== "string") {
    throw new TypeError(
      "classifyEntityType: input must be a non-null string"
    );
  }
 ...
 */
export function shouldRequireSkipTrace(input: {
