...
import { z } from "zod";
import { ENTITY_TYPES, type EntityType } from "./index";
 ...
 */
export const tolerantOptionalEmail = z
  .preprocess(emptyToUndefined, z.string().email("Invalid email").optional())
 ...
 */
export const tolerantOptionalPhone = z
  .preprocess(emptyToUndefined, z.string().max(20).optional())
...
const entityTypeEnum = z.enum(
  ENTITY_TYPES as readonly [EntityType, ...EntityType[]],
);
 ...
 */
export const createLeadContactContract = z
  .object({
    firstName: z.string().max(100).optional(),
    lastName: z.string().max(100).optional(),
    email: tolerantOptionalEmail,
    phone: tolerantOptionalPhone,
  ...
  .refine(
    (data) => {
      if (data.entityType && data.entityType !== "INDIVIDUAL") return true;
      const hasFirst =
    ...
    {
      message:
        "Provide a first and last name, or at least a phone number or first name, for INDIVIDUAL leads.",
      path: ["firstName"],
    },
  )
  ...
  .refine(
    (data) => {
      if (!data.entityType || data.entityType === "INDIVIDUAL") return true;
      return (
        typeof data.entityName === "string" && data.entityName.trim().length > 0
      );
    },
    {
